"""DOCX round-trip regression harness.

tests/fixtures/docx/real/ 하위 DOCX 에 대해:
...
WORK = SCRIPT_DIR / "_poc_out" / "docx_regression"
BASELINE = SCRIPT_DIR / "docx_regression_baseline.json"
...
def extract_metrics(path: Path) -> Metrics:
    """python-docx 의 `id(cell._tc)` 는 GC 재사용으로 비결정적이라, 병합 수는
    XML 속성 (gridSpan / vMerge="restart") 기반으로 카운팅한다."""
    try:
        from docx import Document
    except ImportError:
        return Metrics(error="python-docx not installed")
    ...
    try:
        ...
        m.paragraph_count = len(doc.paragraphs)
        text_buf: list[str] = []
        ...
        for table in doc.tables:
            ...
            m.total_rows += len(rows)
            cols = max((len(tr.findall(f"{W_NS}tc")) for tr in rows), default=0)
            m.total_cols += cols
            ...
            for tr in rows:
                for tc in tr.findall(f"{W_NS}tc"):
                    ...
                    if tc_pr is None:
                        continue
                    # 가로 병합 anchor: <w:gridSpan w:val="N"/> N>1
                    grid_span = tc_pr.find(f"{W_NS}gridSpan")
                    gs = 1
                    if grid_span is not None:
                        try:
                            gs = int(grid_span.get(f"{W_NS}val", "1"))
                        except (TypeError, ValueError):
                            gs = 1
                    # 세로 병합 anchor: <w:vMerge w:val="restart"/>
                    v_merge = tc_pr.find(f"{W_NS}vMerge")
                    is_vmerge_anchor = (
                        v_merge is not None
                        and v_merge.get(f"{W_NS}val") == "restart"
                    )
                    if gs > 1 or is_vmerge_anchor:
                        merged += 1

            m.merged_cell_count += merged

            # 셀 텍스트는 iter() 로 t 수집
            for t in tbl_elem.iter(f"{W_NS}t"):
                if t.text:
...
def verify_fixture(path: Path) -> FixtureResult:
    ...
    try:
        ...
        sentinel = "__DOCX_ADAPTER_EDIT_CHECK_2026_04_17__"
        adapter = load(path)
        try:
            ...
            if target is None:
                result.errors.append("edit: 편집 가능 anchor 셀 없음")
                return result
        ...
        if m_edit.error is None:
            result.edit_structure_preserved = (
                m_orig.paragraph_count == m_edit.paragraph_count
...
def print_result(r: FixtureResult) -> None:
    all_ok = (
        r.bytes_copy_identical and r.lxml_rt_reopen and r.adapter_rt_reopen
        and r.edit_value_roundtrip and r.edit_structure_preserved
        and metrics_match(r.metrics_original, r.metrics_lxml)
        and metrics_match(r.metrics_original, r.metrics_adapter)
    )
    status = "✅" if all_ok else ("⚠️" if r.python_docx_open else "❌")
    ...
    if m.error:
        ...
        return
    print(
        f"    원본: paragraphs={m.paragraph_count}, tables={m.table_count}, "
    ...
    if r.edit_target:
        ...
        print(f"      target=T{t}({row},{col}), old={old_disp!r}, observed={r.edit_observed_value!r}")
    if r.errors:
        for e in r.errors:
            print(f"    ERR: {e}")
...
def main() -> int:
    ...
    print("\n" + "=" * 70)
    total = len(results)
    opened = sum(1 for r in results if r.python_docx_open)
    ...
    print(f"  python-docx로 원본 열림        : {opened}/{total}")
